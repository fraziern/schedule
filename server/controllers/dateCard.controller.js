...
var DateCardController = (function() {
  ...
  function updateLabel(req, res) {
    if (!req.params.cardid || req.body.label == undefined) {
      return res
        .status(404)
        .json({ error: "Some parameters missing" })
        .end();
    }

    var dateCardId = req.params.cardid;
    var update = { $set: { label: req.body.label } };
    DateCard.findByIdAndUpdate(dateCardId, update, { new: true }, function(
      err,
      saved
    ) {
      if (err) return res.status(500).send(err);
  ...
  function addSlotToCard(req, res) {
    if (!req.params.cardid || !req.body.slot) {
      ...
      return res
        .status(404)
        .json({ error: "Some slot parameters missing" })
        .end();
    }
  ...
  function deleteSlotFromCard(req, res) {
    ...
    DateCard.findOneAndUpdate(query, update, { new: true }, function(
      err,
      saved
    ) {
      if (err) return res.status(500).send(err);
  ...
  function updateAllSlots(req, res) {
    if (!req.params.cardid || !req.body.slots)
      return res
        .status(404)
        .json({ error: "Some parameters missing" })
        .end();
    ...
    var dateCardId = req.params.cardid;
    var update = { $set: { slots: req.body.slots } };
